...
A sub C compiler
## Usage
* Write the Code in the `testFile/newtest.txt`.
```c
#testFile/newtest.txt
...
* Back to the shell
```shell
# want to transfer the subc to the llvm IR but no need to run
make test
# want to get the llvm IR and run the code write by the sub c
make run
# just compile
make
# remove the file produced during the compile
make clean
```

* We will get a llvm IR file named `testFile/IR.txt` just like that
```txt
; ModuleID = 'main'
